...
def execute_lftp(lftp_config: dict) -> None:
    ...
    mirror {lftp_config.get("newer_than", "")} {lftp_config.get("include")} --parallel=10 {lftp_config.get("remote_path")} {lftp_config.get("local_path")}; \
    exit"
    """
...
def main():
    ...
    "include": " ".join([f"--include {os.getenv("REMOTE_PATH")}/{dir}" for dir in ["world", "world_nether", "world_the_end"]]),
